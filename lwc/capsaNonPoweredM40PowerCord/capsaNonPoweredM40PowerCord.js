...
export default class CapsaNonPoweredM40PowerCord extends LightningElement {
    ...
    connectedCallback() {
        getOptionsFresh({ stepKey: 'M40PowerCord', productFamily: this.productType || 'NonPoweredM40' })
            .then(data => {
                this._options = (data || []).map(opt => ({ ...opt, isSelected: false, rowClass: 'option-row' }));
                if (this.selections && this.selections.PowerCordOption && this.selections.PowerCordOption.length > 0) {
                    this.selectedKey = this.selections.PowerCordOption[0].key;
                }
                this.isLoading = false;
            })
            .catch(error => {
                console.error('Error fetching Power Cord options:', error);
                this.isLoading = false;
            });
    }
    ...
    handleNext() {
        ...
        this.dispatchEvent(new CustomEvent('stepcomplete', { detail: { PowerCordOption: payload } }));
    }
}