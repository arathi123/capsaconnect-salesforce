...
export default class CapsaRobotsResult extends NavigationMixin(LightningElement) {
    @api opportunityId;
    @api summaryData = {};
    @api quoteId;
    @track isLoading = false;

    get hasKLRobot()      { return (this.summaryData.klRobot || []).length > 0; }
    get hasWarranty()     { return (this.summaryData.warranty || []).length > 0; }
    get hasService()      { return (this.summaryData.service || []).length > 0; }
    ...
    generateProductData() {
        ...
        addItems(this.summaryData.klRobot);
        addItems(this.summaryData.warranty);
        addItems(this.summaryData.service);
    ...
    handleQuoteRequest() {
        ...
        createQuote({ configJson, opportunityId: this.opportunityId, cartSKU: '' })
            ...
            .catch(error => {
                this.isLoading = false;
                console.error('Robots Quote Error:', error);
    ...
    handleUpdateQuote() {
        ...
        updateExistingQuote({ quoteId: this.quoteId, configJson, cartSKU: '' })
            ...
            .catch(error => {
                this.isLoading = false;
                console.error('Robots Update Error:', JSON.stringify(error));
