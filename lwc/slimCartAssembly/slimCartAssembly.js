...
export default class SlimCartStorage extends LightningElement {
    ...
    @wire(getOptionsByStepAndFamily, { stepKey: 'SLIM CART ASSEMBLY', productFamily: '$productType' })
    wiredOptions({ error, data }) {
        if (data) {
            ...
            if (this.selections && this.selections.Assembly && this.selections.Assembly.length > 0) {
                this.localSelection = this.selections.Assembly[0].key;
                this.syncUI();
            }
